...
-- A formula p is a tautology if it is satisfied by all valuations.
tautology :: Form -> Bool
tautology p = all (\v -> evl v p) (allVals p)

-- A formula P logically entails Q is true if and only if all the valuations that satisfy P also satisfy Q.
entails :: Form -> Form -> Bool
entails p q = all (`evl` q) (filter (`evl` p) (allVals p))

-- Formula p and q are equivalent if they produce the same outcomes for all valuations.
...
testContradiction f = (contradiction f == all (\v -> not (evl v f)) (allVals f))

testTautology :: Form -> Bool
testTautology f = (tautology f == all (\v -> evl v f) (allVals f))
...
nsub :: Form -> Int
nsub f = nsub2 0 (sub f)

nsub2 :: Int -> Set Form -> Int
nsub2 n (Set []) = n
nsub2 n (Set (x:xs)) = nsub2 (n + 1) (Set xs)

printnsub :: Form -> IO()
printnsub f = putStrLn ("Formula " ++ show f ++ " contains " ++ (show $ nsub f))
